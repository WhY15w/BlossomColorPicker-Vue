<template>
  ...
  >
    ...
    >
      ...
      <!-- Dynamic Layers -->
      <template v-for="(layerColors, layerIdx) in layers" :key="layerIdx">
        <template
        ...
        >
          ...
          <template v-if="index === getBottomIndex(layerColors.length)">
            ...
            <Petal
              ...
              :animation-duration="animationDuration"
              :stagger-delay="layerPrefixCounts[layerIdx] * PETAL_STAGGER + index * PETAL_STAGGER"
              :z-index="
                getPetalZIndex(
                  index,
              ...
              "
              clip="left"
              :is-external-hover="hoveredPetal?.layer === layerIdx && hoveredPetal?.index === index"
              pointer-events="none"
              :has-shadow="false"
            ...
            <Petal
              ...
              :animation-duration="animationDuration"
              :stagger-delay="layerPrefixCounts[layerIdx] * PETAL_STAGGER + index * PETAL_STAGGER"
              :z-index="(layers.length - layerIdx) * 100 + layerColors.length + 20"
              :alpha="0"
              :has-shadow="false"
              :rotation-offset="layerRotations[layerIdx]"
...
const normalizedColors = computed(() =>
  props.colors && props.colors.length > 0 ? props.colors.map(parseColor) : DEFAULT_COLORS,
)

const layers = computed(() => organizeColorsIntoLayers(normalizedColors.value))
...
const currentValue = computed<BlossomColorPickerValue>(() => props.value ?? internalValue.value)
...
const barRadius = computed(() =>
  calculateBarRadius(layerRadii.value, props.petalSize, props.coreSize, BAR_GAP),
)

const containerSize = computed(() =>
  calculateContainerSize(barRadius.value, BAR_WIDTH, props.showAlphaSlider, SLIDER_OFFSET),
)
...
watch(isExpanded, (expanded) => {
  if (prevExpanded.value && !expanded) {
    ...
    const selectedPetal = allColors.value.find((c) => c.h === currentValue.value.hue)
...
const baseSaturation = computed(() => {
  ...
  const selectedColor = allColors.value.find((c) => c.h === currentValue.value.hue)
  return selectedColor?.s ?? 70
})

const currentLightness = computed(
  () => currentValue.value.lightness ?? (currentValue.value.layer === 'inner' ? 85 : 65),
)

const coreColor = computed(() => {
  if (isExpanded.value && !props.showCoreColor) return '#FFFFFF'
  const lightness =
    currentValue.value.lightness ?? sliderValueToLightness(currentValue.value.saturation)
  const saturation = currentValue.value.originalSaturation ?? baseSaturation.value
...
function handleSliderChange(sliderValue: number) {
  const lightness = sliderValueToLightness(sliderValue)
  const visualSaturation = getVisualSaturation(sliderValue, baseSaturation.value)
...
function handlePetalClick(color: { h: number; s: number; l: number }, layerIdx: number) {
