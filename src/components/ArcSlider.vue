...
const svgSize = computed(() => (arcRadius.value + handleRadius.value + strokeWidth.value) * 2 + 20)
const center = computed(() => svgSize.value / 2)

const handleAngle = computed(
  () => valStartAngle.value + (props.value / 100) * (valEndAngle.value - valStartAngle.value),
)

const handlePos = computed(() =>
  polarToCartesian(center.value, center.value, arcRadius.value, handleAngle.value),
)
...
const handleSaturation = computed(() => getVisualSaturation(props.value, props.baseSaturation))
...
const gradientStart = computed(() =>
  polarToCartesian(center.value, center.value, arcRadius.value, valStartAngle.value),
)
const gradientEnd = computed(() =>
  polarToCartesian(center.value, center.value, arcRadius.value, valEndAngle.value),
)
